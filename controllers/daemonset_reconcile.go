...
const (
	...
	daemonSetImage = "quay.io/cadapa/deamonset:s390x-24"
)
...
// Returns nil if ConfigMap doesn't exist or addon is not configured
func (r *KataConfigOpenShiftReconciler) getAddonEnvVars() []corev1.EnvVar {
	ctx := context.Background()

	// Try to get the addon ConfigMap
	...
	// Check if addon image is configured
	addonImage, exists := configMap.Data["addonImage"]
	if !exists || addonImage == "" {
		// No addon image configured
		return nil
	}

	r.Log.Info("Addon artifacts configured", "image", addonImage)
	...
	if kernelPath, exists := configMap.Data["kernelPath"]; exists && kernelPath != "" {
		envVars = append(envVars, corev1.EnvVar{
			Name:  "ADDON_KERNEL_PATH",
			Value: kernelPath,
		})
	}

	// Add optional initrd path
	if initrdPath, exists := configMap.Data["initrdPath"]; exists && initrdPath != "" {
		envVars = append(envVars, corev1.EnvVar{
			Name:  "ADDON_INITRD_PATH",
			Value: initrdPath,
		})
	}
	return envVars
...
// extractVersionFromImage extracts version tag from container image reference
func extractVersionFromImage(image string) string {
	// Split by colon to get tag
	parts := strings.Split(image, ":")
	if len(parts) > 1 {
		tag := parts[len(parts)-1]
		// Don't use sha256 digest as version
